...
import { validateAllImages, logImageErrors } from './imageValidator';
import { exportToPdf } from './pdfExporter';
import { validateMergeDocuments } from './structuredDocValidator';
import {
...
} from './templateDocxExporter.js';
import { fetchImageAsArrayBuffer } from './imageHandler';
import { getDefaultPenutupData } from './penutupSchema.js';
...
export const generateMonthlyReport = async ({
    ...
    try {
        ...
        const approved = laporan.filter(r => r.laporan.structured_json.pages.length > 0);
            ...
            if (action === 'preview') if(showMsg) showMsg('info', 'Memuat preview...');
            else if(showMsg) showMsg('info', 'Membuat dokumen...');
            ...
            if (action === 'preview') {
                ...
                if(onPreview) onPreview(result.value);
                return;
            }
